...
class BookEpub:
    ...
    def epub_unpack(self, b_path=None, b_pwd=None):
        ...
        """
        if b_path != None:
            b_path = os.path.dirname(self.filename)
        b_name = os.path.splitext(os.path.basename(self.filename))[0]
    ...
    def epub_get_text_from_parts(self, part_name=None):
        ...
        text = re.sub("\[\d+\]", "", text)  # clean text from links like [123]
        return(text)
    ...
    def epub_get_book_txt(self, book_extracted_path=None):
        ...
        book_text = list()
        part_names = self.epub_get_xhtml_names()
        for name in part_names:
            book_text.append(self.epub_get_text_from_parts(book_extracted_path + "/" + name))
